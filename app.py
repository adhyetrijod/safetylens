...
.tb{background:#0f2340;height:52px;display:flex;align-items:center;
  justify-content:space-between;padding:0 2rem;border-bottom:2px solid #1e90ff}
.tb-brand{font-size:1.05rem;font-weight:800;color:#fff}
.tb-brand span{color:#1e90ff}
.tb-sub{font-size:.68rem;color:#7a9ab8;margin-left:8px;text-transform:uppercase;letter-spacing:.08em}
.tb-pill{background:rgba(30,144,255,.18);border:1px solid rgba(30,144,255,.45);
  color:#60b4ff;font-size:.72rem;padding:4px 14px;border-radius:20px;font-weight:600}

.navwrap{background:#fff;border-bottom:1px solid #e2e8f0;padding:0}
...
.body{padding:0.25rem 2.5rem 4rem}
h1,h2,h3{color:#0f2340!important;font-weight:700!important}
.stMarkdown p{color:#1e2d3d!important;font-size:.88rem!important}
[data-testid="stMetricLabel"]{color:#4b6080!important;font-size:.68rem!important;font-weight:700!important;text-transform:uppercase!important;letter-spacing:.07em!important}
[data-testid="stMetricValue"]{color:#0f2340!important;font-size:1.85rem!important;font-weight:800!important}

.body div.stButton>button{border-radius:7px!important;font-weight:600!important;font-size:.84rem!important;min-height:44px!important}
.body div.stButton>button[kind="primary"]{background:#1e90ff!important;color:#fff!important;border:none!important}
.body div.stButton>button[kind="secondary"]{background:#fff!important;color:#1e2d3d!important;border:1px solid #d1dce6!important}
.stTabs [data-baseweb="tab-list"]{border-bottom:2px solid #e2e8f0!important;gap:0}
.stTabs [data-baseweb="tab"]{background:transparent!important;border:none!important;color:#64748b!important;font-weight:600!important;font-size:.83rem!important;padding:.6rem 1.2rem!important}
...
HIERARCHY = {
    "Steel & Metal": {
        ...
        "Tata Metaliks — Kharagpur, WB": {
            "Tata Metaliks Plant": {
                "Blast Furnace (Pig Iron)": "Tata Steel (Integrated)",
                "Ductile Iron Pipe Plant": "Tata Steel (Integrated)",
                "SG Iron Foundry": "Tata Steel (Integrated)",
            },
        },
        "JSW Steel — Vijayanagar, Karnataka": {
            "Vijayanagar Works (17.5 MTPA)": {
                "Blast Furnace Complex (BF1-BF5)": "Tata Steel (Integrated)",
                "BOF Steelmaking": "Tata Steel (Integrated)",
                "Hot Strip Mill (HSM-1 & HSM-2)": "Tata Steel (Integrated)",
                "Cold Rolling Mill": "Steel / Tin Plating (ETL-1)",
                "Hot-Dip Galvanising Lines": "JSW Steel (Galvanising)",
                "Colour Coating Line": "JSW Steel (Galvanising)",
                "Wire Rod & Bar Mill": "Tata Steel (Integrated)",
            },
        },
        ...
        "JSW — Bhushan Power & Steel, Odisha": {
            "Rengali Works (3.5 MTPA)": {
                "Blast Furnace Complex": "Tata Steel (Integrated)",
                "BOF & Caster": "Tata Steel (Integrated)",
                "Hot Rolling Mill": "Tata Steel (Integrated)",
                "Cold Rolling & CRCA": "Steel / Tin Plating (ETL-1)",
            },
        },
        ...
        "SAIL — Rourkela, Odisha": {
            "Rourkela Plant (4.5 MTPA)": {
                "Blast Furnace": "Tata Steel (Integrated)",
                "BOF & Plate Mill": "Tata Steel (Integrated)",
                "Silicon Steel (CRGO)": "Steel / Tin Plating (ETL-1)",
    ...
    "Pharma": {
        "API Manufacturing": {"Units": {
            ...
            "Hydrogenation Suite": "Pharmaceuticals",
        }},
    },
    "Oil & Gas": {
...
PAGES = ["Home","Dashboard","Chemicals","Parameters","Playground","Simulation","Risk Matrix","PSM Report"]

for k,v in {
    "sec":None,"comp":None,"area":None,"div":None,"dkey":None,
    "loaded":False,"pdisplay":"",
...
def rst():
    for k,v in {"sec":None,"comp":None,"area":None,"div":None,"dkey":None,
                "loaded":False,"pdisplay":"","sim":None,"smeta":None,
                "chem":pd.DataFrame(),"param":pd.DataFrame(),
                "proc":pd.DataFrame(),"cim":pd.DataFrame()}.items():
...
st.markdown(f'<div class="tb"><div><span class="tb-brand">Safety<span>Lens</span></span><span class="tb-sub">Process Safety Platform</span></div><span class="tb-pill">{pill}</span></div>',unsafe_allow_html=True)

# NAV — radio horizontal (no overlap)
show_pages = PAGES if st.session_state.loaded else ["Home"]
cur_idx = show_pages.index(st.session_state.page) if st.session_state.page in show_pages else 0
sel = st.radio("nav", show_pages, index=cur_idx, horizontal=True, label_visibility="collapsed")
...
if st.session_state.page == "Home":
    ...
    for col,(s,_) in zip(sc,HIERARCHY.items()):
        with col:
            if st.button(s,key=f"s_{s}",use_container_width=True,
                         type="primary" if st.session_state.sec==s else "secondary"):
                st.session_state.update({"sec":s,"comp":None,"area":None,"div":None}); st.rerun()
        ...
        for col,a in zip(ac,areas):
            with col:
                if st.button(a,key=f"a_{a}",use_container_width=True,
                    ...
                    st.session_state.update({"area":a,"div":None}); st.rerun()
        ...
        for col,(d,dk) in zip(dc,divs.items()):
            with col:
                if st.button(d,key=f"d_{d}",use_container_width=True,
                    ...
                    st.session_state.update({"div":d,"dkey":dk}); st.rerun()
        ...
        if st.button("Load custom plant",type="primary"):
            if cf and pf:
                ...
                st.session_state.update({"loaded":True,"pdisplay":pn or "Custom Plant",
                    ...
                    "cim":d["cim"],"page":"Dashboard"}); st.rerun()
            else: st.error("Upload both files.")

elif not st.session_state.loaded:
    st.info("Go to Home and select a facility.")
    ...
    if PG=="Dashboard":
        ...
        c1.metric("Risk Index",f"{int(RK)}/100"); c2.metric("Processes",len(PR))
        c3.metric("High Hazard",hho); c4.metric("Chemicals",len(C)); c5.metric("Parameters",len(P))
        st.divider()
        if not P.empty:
        ...
        with cl:
            ...
            if not PR.empty and "risk_score" in PR.columns:
                ...
                fig.update_layout(**bl(height=280,xaxis=dict(range=[0,115],showgrid=True,gridcolor="#f1f5f9",zeroline=False),yaxis=dict(title="")))
        ...
        with cr:
            ...
            if not C.empty and "risk_score" in C.columns:
                ...
                st.plotly_chart(fig,use_container_width=True,config=cfg())
        if not P.empty:
            st.divider(); st.markdown("**Top 10 parameters by risk**")
        ...
        if st.button("Change facility"): rst()
        ...
        with t1:
            if not C.empty:
                sel2=st.selectbox("Chemical",C["name"].tolist())
                row=C[C["name"]==sel2].iloc[0]; rs=int(row.get("risk_score",0))
        ...
        if not P.empty:
            ...
            x1.metric("Showing",len(df)); x2.metric("Critical",len(df[df["risk_score"]>=70]))
            x3.metric("Elevated",len(df[(df["risk_score"]>=40)&(df["risk_score"]<70)])); x4.metric("Low",len(df[df["risk_score"]<40]))
        ...
        if not P.empty:
            ...
            fig.update_layout(**bl(height=300,xaxis=dict(title="Reading #",showgrid=True,gridcolor="#f1f5f9"),
                yaxis=dict(title=pr.get("uom",""),showgrid=True,gridcolor="#f1f5f9")))
            st.markdown("**SPC control chart — 30 readings**")
            st.plotly_chart(fig,use_container_width=True,config=cfg())
        ...
        if not P.empty:
            ...
            with c2:
                ...
                if st.session_state.sim:
                    res=st.session_state.sim; pm,pu,s_sc,s_sh,s_sl,s_oh,csq,snm=st.session_state.smeta
                    ...
                    elif soc_b: st.warning(f"SOC breach at step {soc_b}.")
                    else: st.success("Within safe limits throughout simulation.")
                    ...
                    if sol_b: fig.add_vline(x=sol_b,line_dash="dash",line_color="#dc2626",annotation_text="SOL breach")
                    if soc_b: fig.add_vline(x=soc_b,line_dash="dash",line_color="#f59e0b",annotation_text="SOC breach")
                    fig.update_layout(**bl(height=380,title=f"{pm} — {snm}",margin=dict(l=0,r=85,t=45,b=0),
                        xaxis=dict(title="Time step",showgrid=True,gridcolor="#f1f5f9"),
                        yaxis=dict(title=pu,showgrid=True,gridcolor="#f1f5f9")))
                    st.plotly_chart(fig,use_container_width=True,config=cfg())
                else: st.info("Configure a scenario and click Run simulation.")

    elif PG=="Risk Matrix":
        st.title("Risk Matrix")
        st.info("Dark red >= 80: engineering controls. Orange 50-79: admin controls. Green < 50: standard monitoring.")
